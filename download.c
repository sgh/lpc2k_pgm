...
void download_begin(void)
{
	int r;

	printf("\r\nEntering Bootloader Mode\r\n");
	hard_reset_to_bootloader();
	printf("Read \"%s\"", file_setting());
	r = read_intel_hex(file_setting());
	if (r < 0) {
		// how are we going to abort???
	}
	printf(": %d bytes\r\n", r);
...
static void download_main(int event)
{
	...
	switch (state) {
	  ...
	  case SYNC_3:
		switch(event) {
		  case BEGIN:
			if (sscanf(crystal_setting(), "%lf", &xtal) != 1) {
				printf("\r\n");
...
*/
void download_rx_term(const char *buf, int num)
{
